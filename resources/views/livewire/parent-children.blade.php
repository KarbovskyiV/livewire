            ...
            <div class="min-w-full align-middle p-4">
                <table class="min-w-full divide-y divide-gray-200 border">
                    ...
                    @foreach($orderProducts as $key => $orderProduct)
                        <tr>
                            <td class="px-2 py-2">
                                <select wire:model="orderProducts.{{ $key }}.product_id" id="orderProducts[{{ $key }}][product_id]" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm">
                            ...
                            <td class="px-2 py-2">
                                <input wire:model="orderProducts.{{ $key }}.quantity" id="orderProducts[{{ $key }}][price]" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm" type="number" />
                            </td>
                            <td class="px-2 py-2">
                                <button wire:click="removeProduct({{ $key }})" type="button" class="px-3 py-2 bg-red-600 rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-500">
                                    Delete
                                </button>
                            </td>
                        </tr>
